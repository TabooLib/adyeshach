...
internal object DefaultManagerHandler {
    ...
    fun onActive() {
        // 公共管理器
        DefaultAdyeshachBooster.api.publicEntityManager.onEnable()
        // 私有管理器
        onlinePlayers.forEach { Adyeshach.api().setupEntityManager(it) }
        // 公共调度器
        Bukkit.getScheduler().runTaskTimer(bukkitPlugin, Runnable {
            DefaultAdyeshachBooster.api.publicEntityManager.onTick()
            DefaultAdyeshachBooster.api.publicEntityManagerTemp.onTick()
        }, 1L, 1L)
        // 私有调度器
        Bukkit.getScheduler().runTaskTimer(bukkitPlugin, Runnable {
    ...
    fun onDisable() {
        // 公共管理器
        DefaultAdyeshachBooster.api.publicEntityManagerTemp.onDisable()
        DefaultAdyeshachBooster.api.publicEntityManager.onDisable()
        DefaultAdyeshachBooster.api.publicEntityManager.onSave()
        // 私有管理器
